...
<table class="widefat updates-table" id="update-plugins-table">
	...
	foreach ( (array) $plugins as $plugin_file => $plugin_data ) {
		...
		if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $cur_wp_version, '>=' ) ) {
			/* translators: %s: WordPress version. */
			$compat = '<br /><span style="color: green;">&#10004;</span> ' . sprintf( __( 'Compatibility with WordPress %s: 100%% (according to its author)' ), $cur_wp_version );
		} else {
			/* translators: %s: WordPress version. */
			$compat = '<br /><span style="color: red;">&#10008;</span> ' . sprintf( __( 'Compatibility with WordPress %s: Unknown' ), $cur_wp_version );
		}
		// Get plugin compat for updated version of WordPress.
		if ( $core_update_version ) {
			if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $core_update_version, '>=' ) ) {
				/* translators: %s: WordPress version. */
				$compat .= '<br /><span style="color: green;">&#10004;</span> ' . sprintf( __( 'Compatibility with WordPress %s: 100%% (according to its author)' ), $core_update_version );
			} else {
				/* translators: %s: WordPress version. */
				$compat .= '<br /><span style="color: red;">&#10008;</span> ' . sprintf( __( 'Compatibility with WordPress %s: Unknown' ), $core_update_version );
			}
		}
